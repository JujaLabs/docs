...
* GMF-F5-D2 Одному уникальному получателю - только 1 “спасибка” в день, если это правило нарушается, пользователь должен получить сообщение об ошибке в формате GMF-D6-2 с текстом “Этому пользователю ты уже отправлял спасибку”.
...
* GMF-F5-D5 В день я могу отправить не более 2х спасибок, если все же попытаюсь, то получу json сообщение об ошибке в формате GMF-D6-2 с текстом “ты можешь отправить не более 2х спасибо в день”. 
* GMF-F5-D6 Я не могу выставить сам себе спасибку, если я попробую это сделать то получу сообщение об ошибке в формате GMF-D6-2 с текстом “Ты сам себя поблагодарить не можешь. Попробуй составь [дейли-рипортик](http://bit.ly/juja-ABD4536)”
* GMF-F5-URL
...
* GMF-F6-D5 За день возможен только 1 турнир, в случае если была попытка выполнить запрос дважды в день, возникает ошибка формате GMF-D6-2 с текстом “За сегодняшний турнир уже были выставлены ачивки”.
* GMF-F6-URL
    ```
...
* GMF-F7-D3 Список активных Хранителей и закрепленных за ними направлений получаем у сервиса [Keepers](https://github.com/JujaLabs/docs/tree/master/architecture/keepers) url /keepers, method GET
...
* GMF-F7-D5 За неделю возможна только одна транзакция, в случае если была попытка выполнить запрос дважды в неделю, возникает ошибка формате GMF-D6-2 с текстом “На этой неделе уже благодарили Хранителей”.
* GMF-F7-URL
    ```
...
### GMF-F9
***GMF-F9 Я как пользователь хочу иметь возможность поприветствовать нового участника.***

* GMF-F9-D1 Приветствие отправляется новому пользователю только один раз. 
* GMF-F9-D2 Выдается 1 джуджик
* GMF-F9-D3 Текст (description) приветственного сообщения "Welcome to JuJa!"
...
**GMF-F10 Я как пользователь хочу иметь возможность сделать interview report с целью делиться опытом прохождения собеседований.**

* GMF-F10-D1 Нет необходимости указывать кому - интервью рипорт я могу отправить только сам себе
* GMF-F10-D2 За каждый интервью рипорт я получаю по 10 джуджиков
* GMF-F10-D3 Допускается отправка нескольких интервью рипортов в день. 
